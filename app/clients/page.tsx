      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center m-4 sm:m-8 lg:m-16">
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-4 sm:mb-6">
            ...
            <span className="text-[#a693e5]">{t("clients.title").split(" ")[1]}</span>
          </h1>
        </div>
        ...
        <div className="bg-gradient-to-r from-gray-900 to-black rounded-lg p-4 sm:p-6 lg:p-8 xl:p-12 mb-12 sm:mb-16 lg:mb-20">
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-center mb-8 sm:mb-12">
            <span className="text-[#bccceb]">{t("clients.numbers").split(" ")[0]}</span>{" "}
            <span className="text-[#a693e5]">{t("clients.numbers").split(" ")[1]}</span>
          </h2>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
            <div className="text-center">
              <div className="flex items-center justify-center">
                <span className="text-[#6c4fce] font-bold text-2xl sm:text-3xl lg:text-4xl">+</span>
                <CountUp
                  from={0}
                  to={50}
                  separator=","
        ...
        <div className="mb-12 sm:mb-16">
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white text-center mb-8 sm:mb-12">
            <span className="text-[#bccceb]">{t("clients.testimonials").split(" ").slice(0, -2).join(" ")}</span>{" "}
            <span className="text-[#a693e5]">{t("clients.testimonials").split(" ").slice(-2).join(" ")}</span>
          </h2>
            ...
            {testimonials.map((testimonial, index) => (
              <SpotlightCard className="custom-spotlight-card" spotlightColor="rgba(45, 53, 89, 0.7)" key={index}>
                <div className="flex items-center mb-4 sm:mb-6">
                  <div>
                    <h3 className="text-base sm:text-lg font-bold text-[#bccceb]">{testimonial.name}</h3>
                    <p className="text-[#6c4fce] text-sm sm:text-base">{testimonial.company}</p>
                  </div>
                </div>
                <p className="text-[#bccceb] italic text-sm sm:text-base">"{testimonial.text}"</p>
              </SpotlightCard>
            ))}
          </div>
        </div>
        ...
        <div className="text-center">
          <div className="bg-gradient-to-r from-yellow-400 to-yellow-500 rounded-lg p-4 sm:p-6 lg:p-8">
            <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold text-black mb-3 sm:mb-4">
              <ShinyText text={t("clients.nextClient")} disabled={false} speed={3} className="text-[#5eabd6]" />
            </h2>
            <p className="text-[#bccceb] mb-4 sm:mb-6 text-sm sm:text-base max-w-2xl mx-auto">
              {t("clients.nextClientDesc")}
            </p>
